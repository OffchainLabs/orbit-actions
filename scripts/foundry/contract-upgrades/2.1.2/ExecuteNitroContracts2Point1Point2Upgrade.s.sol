...
contract ExecuteNitroContracts2Point1Point2UpgradeScript is Script {
  function run() public {
    NitroContracts2Point1Point2UpgradeAction upgradeAction = NitroContracts2Point1Point2UpgradeAction(
    ...
    bytes memory upgradeCalldata = abi.encodeCall(
      NitroContracts2Point1Point2UpgradeAction.perform,
      (bridge, proxyAdmin)
    );
    ...
    require(
      IERC20Bridge(bridge).nativeTokenDecimals() == 18,
      'Unexpected native token decimals'
    );
  }
}
