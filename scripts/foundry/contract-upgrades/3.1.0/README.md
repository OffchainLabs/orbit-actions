...
To upgrade to Nitro contracts 3.1.0 (BoLD), you will need to use the [BOLDUpgradeAction](https://github.com/OffchainLabs/nitro-contracts/blob/main/src/rollup/BOLDUpgradeAction.sol) from the [nitro-contracts](https://github.com/OffchainLabs/nitro-contracts) repo. 
...
Note that contracts without code changes are not upgraded. It is normal to have some contracts still in the old version after the upgrade as they are equivalent to the new version. There are no associated ArbOS upgrades for this version.

## Requirements

This upgrade only support upgrading from the following [nitro-contracts release](https://github.com/OffchainLabs/nitro-contracts/releases):
...
Another version of this upgrade guide with more BoLD-specific detail is also available in the [Arbitrum docs](arbitrum-docs/launch-orbit-chain/04-maintain-your-chain/05-upgrade-to-bold.mdx).
