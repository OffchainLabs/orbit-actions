...
4. Edit `scripts/files/configs/custom.ts`, be very careful as they will override the existing configuration of your chain. Note: You will also have to add the `EspressoTEEVerifier` address. You can get it by calling the following command (replace `SequencerInboxProxyAddress` with the address of your SequencerInbox proxy):
...
10. Run the following command to update the `EspressoTEEVerifier` address in SequencerInbox contract. Note: The `PARENT_CHAIN_UPGRADE_EXECUTOR` is the address of the upgrade executor contract on the parent chain, the `SequencerInboxProxyAddress` is the address of the SequencerInbox proxy contract on the parent chain, and the `EspressoTEEVerifierAddress` is the address of the EspressoTEEVerifier contract on the child chain which is the same address you put in custom.ts.
...
11. Update your nodes configuration and restart them:
